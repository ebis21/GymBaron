    ...
    <div className="screen management-screen management-marketing">
      ...
      <div className="management-grid campaign-grid">
        {CAMPAIGNS.map(campaign => {
          ...
          return (
            <article
              className={`management-card campaign-card tone-${campaign.id}${reason && !isRunning ? ' is-locked' : ''}${isRunning ? ' is-live' : ''}`}
              key={campaign.id}
            >
              ...
              <div className="campaign-metrics">
                ...
                {projecting && <span>{t.marketing.projected(arrivals)}</span>}
                {projecting && (
                  <span className={`campaign-net ${net >= 0 ? 'good' : 'bad'}`}>
                    {net >= 0
