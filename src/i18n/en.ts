...
export const en = {
  ...
  topbar: {
    ...
    cash: 'Cash',
    members: 'Members',
    payday: (days: number) => `payday in ${days} ${days === 1 ? 'day' : 'days'}`,
    paydayToday: (amount: string) => `payday today +${amount}`,
    paydayTodayEmpty: 'payday today',
    gymClass: 'Class',
  ...
  phone: {
    ...
    apps: {
      ...
      staff: 'Staff',
      upgrades: 'Upgrades',
      'diamond-upgrades': 'Diamond upgrades',
      multiplayer: 'Friends',
      account: 'Account',
      marketing: 'Marketing',
      contracts: 'Contracts',
  ...
  report: {
    ...
    passes: 'New passes',
    payday: (count: number, each: string) => `Pass collection (${count} × ${each})`,
    paydayHint: (days: number) =>
      `Passes are collected in ${days} ${days === 1 ? 'day' : 'days'} — every member pays at once.`,
    total: 'Total',
